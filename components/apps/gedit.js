...
export class Gedit extends Component {
    ...
    sendMessage = async () => {
        ...
        if (email.length === 0) {
            $("#sender-email").val('');
            $("#sender-email").attr("placeholder", "Email must not be Empty!");
            error = true;
        }
        ...
        try {
            const response = await fetch('https://formspree.io/f/mvggerrp', {
                method: 'POST',
                headers: {
        ...
        return (
            <div className="w-full h-full relative flex flex-col bg-ub-cool-grey text-white select-none">
                ...
                <div className="relative flex-grow flex flex-col bg-ub-gedit-dark font-normal windowMainScreen">
                    ...
                    <div className="relative">
                        <input id="sender-email" className=" w-full my-1 text-ubt-gedit-blue focus:bg-ub-gedit-light gedit-subject outline-none text-sm font-normal pl-6 py-0.5 bg-transparent" placeholder="Your Email:" spellCheck="false" autoComplete="off" type="email" />
